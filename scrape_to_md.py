...
def fetch_jalur_data(jalur):
    base_url = 'https://spmb.kotabogor.go.id/jenjang-smp/data-spmb/data-pendaftar/detail/6058e604-2df5-e011-91f0-e9feaa2e9d74'
    url = f'{base_url}?filter=semua&jalur={jalur}&limit=200&page=1'
    ...
    try:
        ...
        if match:
            ...
            print(f'Warning: data-page not found for {jalur}')
            return None
    except Exception as e:
        print(f'Error fetching {jalur}:', e)
        return None
...
def main():
    ...
    # We will fetch one first to get school details
    first_props = fetch_jalur_data('mutasi') # Mutasi is small, quick
    if not first_props:
        print('Error: Could not retrieve initial data.')
        return
        
    school = first_props.get('satuanPendidikan', {})
    school_name = school.get('nama_satuan_pendidikan', 'SMP NEGERI 1 BOGOR')
    npsn = school.get('npsn', '20220385')
    jumlah_terverifikasi = school.get('jumlah_terverifikasi', 317)
    last_update = school.get('terakhir_diperbarui', '')
    
    print(f'School: {school_name} (NPSN: {npsn})')
    ...
    for jalur in jalurs:
        ...
        props = fetch_jalur_data(jalur)
        if props:
            all_props[jalur] = props
            
    # Write SMP Negeri 1 Bogor overview
    overview_content = f"""# {school_name} - SPMB Bogor 2026
...
"""
    ...
    with open(raw_dir / 'smp_negeri_1_bogor.md', 'w', encoding='utf-8') as f:
        f.write(overview_content)
    print('Created smp_negeri_1_bogor.md')
    ...
    for jalur in jalurs:
        ...
        md_content = f"""# Jalur {pathway_name} - {school_name}
