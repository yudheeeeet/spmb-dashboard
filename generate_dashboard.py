    ...
    <script>
        ...
        function handleUploadedFile(file) {
            ...
            reader.onload = function(e) {
                try {
                    const data = json_parse = JSON.parse(e.target.result);
                    if (data.schools) {
                        spmbData = data;
                        document.getElementById('cors-alert').style.display = 'none';
                        initDashboard();
        ...
        function switchPathway(pathwayKey) {
            ...
            } else if (pathwayKey === 'domisili') {
                sortConfig = { key: 'jarak', direction: 'asc' }; // shorter distance is better
            } else {
                sortConfig = { key: 'jarak', direction: 'asc' };
            }
        ...
        function switchSubPathway(subPathwayKey) {
            ...
            if (subPathwayKey === 'akademik') {
                sortConfig = { key: 'skor_sertifikat', direction: 'desc' };
            } else {
                sortConfig = { key: 'nilai_rapor', direction: 'desc' };
            }
...
"""
    ...
    # Just save the html template directly into index.html, since we load pendaftar_data.json dynamically!
    with open('index.html', 'w', encoding='utf-8') as f:
        f.write(html_template)
    print("Created index.html (dynamic loader) successfully!")
    
    # Also write generate_dashboard.py itself so it matches
    # Wait, we just overwrite generate_dashboard.py with a template that saves index.html
    # This is perfect!
